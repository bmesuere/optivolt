export function parseSolution(result, cfg, opts = {}) {
  ...
  const timelineMs = Array.isArray(opts.timestampsMs) ? opts.timestampsMs : null;
  const startMs = Number.isFinite(opts.startMs) ? opts.startMs : null;
  ...
  let timestampsMs;
  if (timelineMs && timelineMs.length === T) {
    timestampsMs = timelineMs.slice();
  } else if (startMs && Number.isFinite(startMs)) {
    timestampsMs = synthesizeFromStart(startMs, stepMin, T);
  } else {
