 ...
 *   4. else                         → selfConsumption + block both
 */
export function mapRowsToDessV2(rows, cfg) {
  ...
  for (let t = 0; t < rows.length; t++) {
    ...
    // Find this slot's segment tipping points
    const seg = getSegmentForIndex(segments, t);
    const gridChargeTp = findHighestGridChargeCost(rows, seg);
    const gridBatteryTp = findHighestGridUsageCost(rows, seg, cfg);
    const batteryExportTp = findLowestGridExportRevenue(rows, seg);
    ...
    if (importCost <= gridChargeTp) {
      ...
      strategy = Strategy.proBattery;
      restrictions = Restrictions.batteryToGrid; // allow grid→battery
      if (gridImport >= cfg.maxGridImport_W - 1 || chargePower >= cfg.maxChargePower_W - 1) {
        socTarget_percent = Math.min(socTarget_percent + 5, cfg.maxSoc_percent - 1);
      }
      ...
      strategy = Strategy.proGrid;
      restrictions = Restrictions.gridToBattery; // allow battery→grid
      if (gridExport >= cfg.maxGridExport_W - 1 || dischargePower >= cfg.maxDischargePower_W - 1) {
