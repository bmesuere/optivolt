...
export function mapRowsToDess(rows, cfg) {
  ...
  for (let t = 0; t < rows.length; t++) {
    ...
    const socTarget_percent = row.soc_percent;
 ...
 */
function isAtSocBoundary(row) {
  const soc = Number(row.soc);
  const atMin = soc <= row.socTarget_percent + SOC_EPSILON_PERCENT;
  const atMax = soc >= row.socTarget_percent - SOC_EPSILON_PERCENT;
  return atMin || atMax;
}
 ...
 * - firstSegmentTippingPoint_cents_per_kWh: highest grid usage price
 *   in the first SoC segment (or null if none).
 */
function computeDessDiagnostics(rows, segments, cfg) {
  if (!Array.isArray(rows) || !rows.length) {
    return { firstSegmentTippingPoint_cents_per_kWh: null };
  }
  const firstSegment = segments[0];
  const tp = findHighestGridUsageCost(rows, firstSegment, cfg);
  return { firstSegmentTippingPoint_cents_per_kWh: tp };
}
