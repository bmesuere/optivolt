...
function isAtSocBoundary(row, cfg) {
  ...
  const minSoc = Number(cfg.minSoc_percent) || 0;
  const maxSoc = Number(cfg.maxSoc_percent);
  const atMin = soc <= minSoc + SOC_EPSILON_PERCENT;
  const atMax = soc >= maxSoc - SOC_EPSILON_PERCENT;
  return atMin || atMax;
}
