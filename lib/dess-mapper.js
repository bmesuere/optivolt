...
function isAtSocBoundary(row, cfg) {
  ...
  const atMin = soc <= cfg.minSoc_percent / 100 * cfg.batteryCapacity_Wh + SOC_EPSILON_Wh;
  const atMax = soc >= cfg.maxSoc_percent / 100 * cfg.batteryCapacity_Wh - SOC_EPSILON_Wh;
  return atMin || atMax;
}
