...
export class VictronMqttClient {
   ...
   */
  async getSerial({ timeoutMs = 5000, debug = false } = {}) {
    if (this.serial) return this.serial;

    if (!this._serialPromise) {
      this._serialPromise = this._detectSerialOnce({ timeoutMs, debug });
    }
    console.log("**** fetching serial ****");
  ...
  async _detectSerialOnce({ timeoutMs = 5000, debug = false } = {}) {
    ...
    const wait = this._waitForFirstMessage(
      ...
      (topic, payload) => {
        if (debug) {
          console.log('[victron-mqtt] message:', topic, payload.toString());
        }
    ...
    try {
      if (debug) {
        console.log('[victron-mqtt] subscribing to', wildcard);
      }
